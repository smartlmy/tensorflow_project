...
def my_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        optimizer = tf.compat.v1.train.AdagradOptimizer(learning_rate=0.1)  # 用它优化损失函数，达到损失最少精度最高
        train_op = optimizer.minimize(loss, global_step=tf.compat.v1.train.get_global_step())  # 执行优化！
...
models_path = '../model/iris_estimator'
